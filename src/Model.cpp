...
int addVertex(TriangleMesh* mesh, tinyobj::attrib_t& attributes,
              const tinyobj::index_t& idx, std::map<int, int>& knownVertices) {
...
void Model::readOBJ(std::string objFilePath) {
    ...
    bool readOK = tinyobj::LoadObj(&attributes, &shapes, &materials, &err, &err,
                                   objFilePath.c_str(), modelDir.c_str(),
                                   /* triangulate */ true);
    if (!readOK) {
        ...
        for (int materialID : materialIDs) {
            ...
            for (int faceID = 0; faceID < shape.mesh.material_ids.size();
                ...
                mesh->material->diffuse =
                    (const glm::vec3&)materials[materialID].diffuse;
                mesh->material->ambient =
                    (const glm::vec3&)materials[materialID].ambient;
                mesh->material->specular =
                    (const glm::vec3&)materials[materialID].specular;
                mesh->material->emmision =
                    (const glm::vec3&)materials[materialID].emission;
                mesh->material->shininess =
                    (const float&)materials[materialID].shininess;
                mesh->material->dissolve =
                    (const float&)materials[materialID].dissolve;
                mesh->material->illuminationModel =
                    (const int&)materials[materialID].illum;
