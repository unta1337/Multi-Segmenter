...
bool writeOBJ(std::vector<TriangleMesh*> meshes, std::string path, bool isOneFile);
...
int main() {
	...
	std::cout << "mesh 1 vertex size : " << tmp.meshes[0]->vertex.size() << std::endl;
	std::cout << "mesh 1 triangle size : " << tmp.meshes[0]->index.size() << std::endl;
...
bool writeOBJ(std::vector<TriangleMesh*> meshes, std::string path, bool isOneFile) {
	...
	if (isOneFile) {
		...
		for (int i = 0; i < meshes.size(); i++) {
			...
			mtlFile << "Ka " << mesh->material->ambient.x << " " << mesh->material->ambient.y << " " << mesh->material->ambient.z << std::endl;
			mtlFile << "Kd " << mesh->material->diffuse.x << " " << mesh->material->diffuse.y << " " << mesh->material->diffuse.z << std::endl;
			mtlFile << "Ks " << mesh->material->specular.x << " " << mesh->material->specular.y << " " << mesh->material->specular.z << std::endl;
		...
		for (int i = 0; i < meshes.size(); i++) {
			...
			for (int i = 0; i < mesh->index.size(); i++) {
				objFile << "f " << vertexLength+mesh->index[i].x << " " << vertexLength+mesh->index[i].y << " " << vertexLength+mesh->index[i].z << std::endl;
			}
		...
		for (int i = 0; i < meshes.size(); i++) {
			...
			for (int i = 0; i < mesh->index.size(); i++) {
				objFile << "f " << mesh->index[i].x << " " << mesh->index[i].y << " " << mesh->index[i].z << std::endl;
			}
