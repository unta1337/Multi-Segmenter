...
namespace FaceGraph {
    ...
    struct Vec3Hash {
        std::size_t operator()(const glm::vec3& v) const {
            // Use the std::hash function to hash the individual components of the vector
            ...
            for (int i = 0; i < _list->size(); i++) {
                for (int j = 0; j < 3; j++) {
                    ...
                    for (int k = 0; k < adjacent_triangles.size(); k++) {
                        ...
                        if (i != adjacent_triangle && isConnected(_list->at(i), _list->at(adjacent_triangle))) {
    ...
    TriangleMesh* TriangleListToObj(std::vector<Triangle_t> __list) {
        ...
        for (int i = 0; i < __list.size(); i++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                //auto vertIter = std::find(subObject->vertex.begin(), subObject->vertex.end(), __list[i].vert[j]);
                ...
                //    index[j] = subObject->vertex.end() - subObject->vertex.begin();
                //}
            }
...
public:
    ...
    Segmentation_t(TriangleMesh* _mesh, float _tolerance) :mesh(_mesh), tolerance(_tolerance){}
    ...
    std::vector<TriangleMesh*> DoSegmentation() {
        ...
        for (int i = 0; i < mesh->index.size(); i++) {
            facenormals[i] = glm::triangleNormal(mesh->vertex[mesh->index[i].x], mesh->vertex[mesh->index[i].y], mesh->vertex[mesh->index[i].z]);
        }
        ...
        for (int i = 0; i < count; i++) {
            glm::vec3 targetNorm = facenormals[i];
             ...
             for (auto iter : countMap) {
                 ...
                 float normAngle = glm::degrees(glm::angle(_compare, targetNorm));
        ...
        std::unordered_map <glm::vec3, std::vector<FaceGraph::Triangle_t>, FaceGraph::Vec3Hash> myMap;
        for (auto &iter : countMap) {
            //std::cout << "map["<< glm::to_string(iter.first) <<"] : " << iter.second << std::endl;
            myMap.insert({iter.first, std::vector<FaceGraph::Triangle_t>(iter.second) });
            //std::cout << "check : " << myMap[iter.first].size() << std::endl;
            iter.second = 0;
        }
        std::cout << "map count complete (map size : " << countMap.size() << ")" << std::endl;
        ...
        std::cout << "map compaction complete (map size : " << countMap.size() << ")" << std::endl;
        ...
        std::cout << "Normal map insert done total (" << myMap.size() << ") size map" << std::endl;
        ...
        for (auto iter : myMap) {
            ...
            //std::cout << "Key[" << iter.first.x << ", " << iter.first.y << ", " << iter.first.z << "] : ";
            //std::cout << "Number of faces : " << iter.second.size() << std::endl;

            FaceGraph::FaceGraph fg(&iter.second);
            std::cout << "Face Graph done" << std::endl;
            std::vector<std::vector<FaceGraph::Triangle_t>> temp = fg.check_connected();
            ...
            for (auto subs : temp) {
                ...
                subObject->material->name = "sub_materials_"+std::to_string(number);
                subObject->name = mesh->name + "_seg_" + std::to_string(number++);
            ...
            TotalTime += (ms / 1000.);
            std::cout << "Spend : " << TotalTime << " ms (" << (ms / 1000.) << " ms)" << std::endl;
        }
        std::cout << "Check connectivity and Make triangle mesh done"  << std::endl;

        for (int i = 0; i < result.size(); i++) {
            result[i]->material->diffuse = Color::getColorfromJET(i, 0, result.size());
